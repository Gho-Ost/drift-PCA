import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import os
import argparse
import json
import logging
import warnings

warnings.filterwarnings("ignore", category=FutureWarning, module="seaborn")
# Workaround for seaborn and pandas >= 2.2
pd.options.mode.chained_assignment = None
try:
    pd.set_option('mode.use_inf_as_na', True)
except pd.errors.OptionError:
    pass
import pandas._config.config as pd_config
try:
    pd_config.register_option('mode.use_inf_as_null', True, 'use_inf_as_null')
except pd_config.OptionError:
    pass

logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
logger = logging.getLogger(__name__)


def create_synthetic_drift(
    name,
    means_pre,
    stds_pre,
    means_post,
    stds_post,
    n_samples=4000,
    data_dir=None,
    vis_dir=None,
    save_to_disk=True
):
    """
    Generate synthetic drift dataset (class-aggregate).
    Infers number of features from means_pre length.
    n_samples is divided equally between pre and post drift.
    """
    n_features = len(means_pre)
    
    # Assertions for input shapes
    assert len(stds_pre) == n_features, "stds_pre must match means_pre length"
    assert len(means_post) == n_features, "means_post must match means_pre length"
    assert len(stds_post) == n_features, "stds_post must match means_pre length"
    
    n_samples_pre = n_samples // 2
    n_samples_post = n_samples - n_samples_pre
    
    # Generate Pre-Drift Data
    X_pre = np.random.normal(loc=means_pre, scale=stds_pre, size=(n_samples_pre, n_features))
    y_pre = np.random.randint(0, 2, size=n_samples_pre)
    
    # Generate Post-Drift Data
    X_post = np.random.normal(loc=means_post, scale=stds_post, size=(n_samples_post, n_features))
    y_post = np.random.randint(0, 2, size=n_samples_post)
    
    # Create DataFrames
    cols = [str(i) for i in range(n_features)]
    df_pre = pd.DataFrame(X_pre, columns=cols)
    df_pre['target'] = y_pre
    df_post = pd.DataFrame(X_post, columns=cols)
    df_post['target'] = y_post
    
    df_pre_plot = df_pre.copy()
    df_pre_plot['Drift_Stage'] = 'Pre-Drift'
    df_post_plot = df_post.copy()
    df_post_plot['Drift_Stage'] = 'Post-Drift'
    df_combined = pd.concat([df_pre_plot, df_post_plot], ignore_index=True)

    if not save_to_disk:
        return X_pre, y_pre, X_post, y_post, df_combined
    
    # Create Directories
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    if data_dir is None:
        data_dir = os.path.join(project_root, "data", "synthetic", "agg")
    if vis_dir is None:
        vis_dir = os.path.join(project_root, "results", "synthetic", "vis")
        
    os.makedirs(data_dir, exist_ok=True)
    os.makedirs(vis_dir, exist_ok=True)
    
    # Save Data
    pre_path = os.path.join(data_dir, f"{name}_pre.csv")
    post_path = os.path.join(data_dir, f"{name}_post.csv")
    df_pre.to_csv(pre_path, index=False)
    df_post.to_csv(post_path, index=False)
    logger.info(f"Saved datasets for {name} to:\n- {pre_path}\n- {post_path}")
    
    # Generating visualization
    g = sns.pairplot(
        df_combined,
        vars=cols,
        hue='Drift_Stage',
        palette={'Pre-Drift': 'blue', 'Post-Drift': 'red'},
        plot_kws={'alpha': 0.5, 's': 15},
        diag_kind='hist'
    )
    g.fig.suptitle(f'Synthetic Drift Dataset: {name}\n(Features: {n_features}, Samples: {n_samples})', y=1.02)
    vis_path = os.path.join(vis_dir, f"{name}_vis.png")
    g.savefig(vis_path, dpi=150)
    plt.close()
    logger.info(f"Saved visualization to: {vis_path}")
    
    return X_pre, y_pre, X_post, y_post, df_combined


def create_synthetic_drift_classes(
    name,
    means_pre_c0, stds_pre_c0,
    means_pre_c1, stds_pre_c1,
    means_post_c0, stds_post_c0,
    means_post_c1, stds_post_c1,
    n_samples=4000,
    data_dir=None,
    vis_dir=None,
    save_to_disk=True
):
    """
    Generate synthetic drift dataset with separate parameters for classes 0 and 1 (class-specific).
    """
    n_features = len(means_pre_c0)
    
    n_samples_pre = n_samples // 2
    n_samples_post = n_samples - n_samples_pre
    
    # Generate balanced classes (50% class 0, 50% class 1) for both pre and post by default
    n_pre_c0 = n_samples_pre // 2
    n_pre_c1 = n_samples_pre - n_pre_c0
    n_post_c0 = n_samples_post // 2
    n_post_c1 = n_samples_post - n_post_c0

    # Generate Pre-Drift Data
    X_pre_c0 = np.random.normal(loc=means_pre_c0, scale=stds_pre_c0, size=(n_pre_c0, n_features))
    X_pre_c1 = np.random.normal(loc=means_pre_c1, scale=stds_pre_c1, size=(n_pre_c1, n_features))
    X_pre = np.vstack([X_pre_c0, X_pre_c1])
    y_pre = np.array([0]*n_pre_c0 + [1]*n_pre_c1)
    
    # Shuffle pre
    idx_pre = np.random.permutation(n_samples_pre)
    X_pre = X_pre[idx_pre]
    y_pre = y_pre[idx_pre]

    # Generate Post-Drift Data
    X_post_c0 = np.random.normal(loc=means_post_c0, scale=stds_post_c0, size=(n_post_c0, n_features))
    X_post_c1 = np.random.normal(loc=means_post_c1, scale=stds_post_c1, size=(n_post_c1, n_features))
    X_post = np.vstack([X_post_c0, X_post_c1])
    y_post = np.array([0]*n_post_c0 + [1]*n_post_c1)
    
    # Shuffle post
    idx_post = np.random.permutation(n_samples_post)
    X_post = X_post[idx_post]
    y_post = y_post[idx_post]
    
    # Create DataFrames
    cols = [str(i) for i in range(n_features)]
    df_pre = pd.DataFrame(X_pre, columns=cols)
    df_pre['target'] = y_pre
    df_post = pd.DataFrame(X_post, columns=cols)
    df_post['target'] = y_post
    
    df_pre_plot = df_pre.copy()
    df_pre_plot['Drift_Stage'] = 'Pre-Drift'
    df_post_plot = df_post.copy()
    df_post_plot['Drift_Stage'] = 'Post-Drift'
    df_combined = pd.concat([df_pre_plot, df_post_plot], ignore_index=True)

    if not save_to_disk:
        return X_pre, y_pre, X_post, y_post, df_combined
    
    # Create Directories
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    if data_dir is None:
        data_dir = os.path.join(project_root, "data", "synthetic", "class")
    if vis_dir is None:
        vis_dir = os.path.join(project_root, "data", "synthetic", "vis")
        
    os.makedirs(data_dir, exist_ok=True)
    os.makedirs(vis_dir, exist_ok=True)
    
    # Save Data
    pre_path = os.path.join(data_dir, f"{name}_pre.csv")
    post_path = os.path.join(data_dir, f"{name}_post.csv")
    df_pre.to_csv(pre_path, index=False)
    df_post.to_csv(post_path, index=False)
    logger.info(f"Saved datasets for {name} to:\n- {pre_path}\n- {post_path}")
    
    # Generating visualization
    df_combined['Stage_Target'] = df_combined['Drift_Stage'] + " - Class " + df_combined['target'].astype(str)
    g = sns.pairplot(
        df_combined,
        vars=cols,
        hue='Stage_Target',
        plot_kws={'alpha': 0.5, 's': 15},
        diag_kind='kde'
    )
    g.fig.suptitle(f'Synthetic Class Drift Dataset: {name}\n(Features: {n_features}, Samples: {n_samples})', y=1.02)
    vis_path = os.path.join(vis_dir, f"{name}_vis.png")
    g.savefig(vis_path, dpi=150)
    plt.close()
    logger.info(f"Saved visualization to: {vis_path}")
    
    return X_pre, y_pre, X_post, y_post, df_combined


def generate_from_json(json_path, n_samples=4000, data_dir=None, vis_dir=None):
    with open(json_path, 'r') as f:
        datasets = json.load(f)
        
    success_count = 0
    for i, params in enumerate(datasets):
        name = params['name']
        logger.info(f"--- Generating scenario {i+1}/{len(datasets)}: {name} ---")
        logger.info(f"Description: {params.get('desc', '')}")
        try:
            # Check if this scenario is class-specific or class-aggregate
            is_class_specific = 'means_pre_c0' in params or 'means_pre_c1' in params
            
            if is_class_specific:
                create_synthetic_drift_classes(
                    name=name,
                    means_pre_c0=params['means_pre_c0'],
                    stds_pre_c0=params['stds_pre_c0'],
                    means_pre_c1=params['means_pre_c1'],
                    stds_pre_c1=params['stds_pre_c1'],
                    means_post_c0=params['means_post_c0'],
                    stds_post_c0=params['stds_post_c0'],
                    means_post_c1=params['means_post_c1'],
                    stds_post_c1=params['stds_post_c1'],
                    n_samples=params.get('n_samples', n_samples),
                    data_dir=data_dir,
                    vis_dir=vis_dir
                )
            else:
                create_synthetic_drift(
                    name=name,
                    means_pre=params['means_pre'],
                    stds_pre=params['stds_pre'],
                    means_post=params['means_post'],
                    stds_post=params['stds_post'],
                    n_samples=params.get('n_samples', n_samples),
                    data_dir=data_dir,
                    vis_dir=vis_dir
                )
            success_count += 1
        except Exception as e:
            logger.error(f"Failed to generate {name}: {e}", exc_info=True)
            
    logger.info(f"Finished generating datasets. {success_count}/{len(datasets)} successful.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate synthetic drift datasets (aggregate or class-specific)")
    parser.add_argument("--json", type=str, default=None, help="Path to JSON file with dataset definitions")
    
    # Class-aggregate arguments
    parser.add_argument("--name", type=str, default=None, help="Custom name for the dataset")
    parser.add_argument("--means_pre", type=float, nargs='+', help="List of means for pre-drift features")
    parser.add_argument("--stds_pre", type=float, nargs='+', help="List of standard deviations for pre-drift features")
    parser.add_argument("--means_post", type=float, nargs='+', help="List of means for post-drift features")
    parser.add_argument("--stds_post", type=float, nargs='+', help="List of standard deviations for post-drift features")
    
    # Class-specific arguments
    parser.add_argument("--means_pre_c0", type=float, nargs='+', help="Means for Class 0 Pre-drift")
    parser.add_argument("--stds_pre_c0", type=float, nargs='+', help="STDs for Class 0 Pre-drift")
    parser.add_argument("--means_pre_c1", type=float, nargs='+', help="Means for Class 1 Pre-drift")
    parser.add_argument("--stds_pre_c1", type=float, nargs='+', help="STDs for Class 1 Pre-drift")
    parser.add_argument("--means_post_c0", type=float, nargs='+', help="Means for Class 0 Post-drift")
    parser.add_argument("--stds_post_c0", type=float, nargs='+', help="STDs for Class 0 Post-drift")
    parser.add_argument("--means_post_c1", type=float, nargs='+', help="Means for Class 1 Post-drift")
    parser.add_argument("--stds_post_c1", type=float, nargs='+', help="STDs for Class 1 Post-drift")
    
    # Common arguments
    parser.add_argument("--n_samples", type=int, default=4000, help="Total number of samples")
    parser.add_argument("--data_dir", type=str, default=None, help="Directory to save CSV data")
    parser.add_argument("--vis_dir", type=str, default=None, help="Directory to save visualizations")
    
    args = parser.parse_args()
    
    # Resolve relative json paths relative to project root if needed
    if args.json is not None:
        json_path = args.json
        if not os.path.exists(json_path):
            project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
            possible_paths = [
                os.path.join(project_root, args.json),
                os.path.join(project_root, "data", args.json)
            ]
            for p in possible_paths:
                if os.path.exists(p):
                    json_path = p
                    break
        
        if os.path.exists(json_path):
            generate_from_json(
                json_path=json_path,
                n_samples=args.n_samples,
                data_dir=args.data_dir,
                vis_dir=args.vis_dir
            )
        else:
            logger.error(f"JSON file not found at {args.json}")
    else:
        if args.name is None:
            parser.error("--name is required when --json is not specified")
            
        # Check which mode we are in based on arguments
        is_class_specific = args.means_pre_c0 is not None or args.means_pre_c1 is not None
        
        if is_class_specific:
            # Check requirements for class-specific single dataset
            required_args = [
                args.means_pre_c0, args.stds_pre_c0,
                args.means_pre_c1, args.stds_pre_c1,
                args.means_post_c0, args.stds_post_c0,
                args.means_post_c1, args.stds_post_c1
            ]
            if any(a is None for a in required_args):
                parser.error("All class-specific lists (means/stds pre/post for c0/c1) must be provided in class-specific mode.")
            
            create_synthetic_drift_classes(
                name=args.name,
                means_pre_c0=args.means_pre_c0,
                stds_pre_c0=args.stds_pre_c0,
                means_pre_c1=args.means_pre_c1,
                stds_pre_c1=args.stds_pre_c1,
                means_post_c0=args.means_post_c0,
                stds_post_c0=args.stds_post_c0,
                means_post_c1=args.means_post_c1,
                stds_post_c1=args.stds_post_c1,
                n_samples=args.n_samples,
                data_dir=args.data_dir,
                vis_dir=args.vis_dir
            )
        else:
            # Check requirements for class-aggregate single dataset
            required_args = [args.means_pre, args.stds_pre, args.means_post, args.stds_post]
            if any(a is None for a in required_args):
                parser.error("All class-aggregate lists (means/stds pre/post) must be provided in class-aggregate mode.")
                
            create_synthetic_drift(
                name=args.name,
                means_pre=args.means_pre,
                stds_pre=args.stds_pre,
                means_post=args.means_post,
                stds_post=args.stds_post,
                n_samples=args.n_samples,
                data_dir=args.data_dir,
                vis_dir=args.vis_dir
            )
