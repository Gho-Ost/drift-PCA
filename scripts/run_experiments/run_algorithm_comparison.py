...
def compute_diff_matrix(X_pre, X_post, y_pre, y_post):
    ...
    return np.array(diff_vectors), classes_computed

def run_comparison():
    parser = argparse.ArgumentParser(description="Run Algorithm Comparison")
    parser.add_argument("--data_dir", type=str, default="data\\real\\comprehensive", help="Directory containing the dataset files")
    parser.add_argument("--results_dir", type=str, default="results\\comparisons", help="Directory to save the results")
    parser.add_argument("--dataset", type=str, default="noaa_drift2", help="Name of the dataset (without _pre.csv)")
    ...
    diff_matrix, classes_computed = compute_diff_matrix(X_pre, X_post, y_pre, y_post)
    ...
    for row_idx, fitter in enumerate(fitters):
        ...
        plot_algorithm_scatter(X_pre_trans, y_pre, ax_pre, f"{fitter.name} - PRE Drift", is_pre=True, classes=classes)
        ax_pre.set_xlim(x_lims)
        ax_pre.set_ylim(y_lims)
        
        # Draw Post Scatter
        ax_post = fig.add_subplot(gs[row_idx, 1])
        plot_algorithm_scatter(X_post_trans, y_post, ax_post, f"{fitter.name} - POST Drift", is_pre=False, classes=classes)
        ...
        if vectors_trans is not None:
            plot_algorithm_drift_compass(vectors_trans, ax_drift, classes=classes, classes_computed=classes_computed)
        else:
